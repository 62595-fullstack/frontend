...
export default function RulesTab() {
  ...
  async function save() {
    ...
    setSaveError(null);
    try {
      await api.updateEvent(event.id, { rules: draft });
      setEvent({ ...event, rules: draft });
      setEditing(false);
    } catch (err) {
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-bg">
      <div className="h-10 bg-bg-dark px-6 py-2 flex items-center justify-between">
        <span className="underline text-text">Tournament Rules</span>
        {isCreator && !editing && (
          <button
            onClick={startEdit}
